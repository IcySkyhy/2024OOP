...
Model3D::Model3D() : m_Faces(), m_Lines(), m_Points(), Faces(m_Faces), Lines(m_Lines), Points(m_Points) {}
...
string Model3D::AddFace(int FaceIndex[]) {
	//通过三个点构造一个面
	//检查三个点是否在范围内
	if (FaceIndex[0] >= m_Points.size() || FaceIndex[1] >= m_Points.size() || FaceIndex[2] >= m_Points.size()) {
...
string Model3D::AddLine(int LineIndex[]) {
	//通过两个点构造一条线
	//检查两个点是否在范围内
	if (LineIndex[0] > m_Points.size() || LineIndex[1] >= m_Points.size() || LineIndex[0] < 1 || LineIndex[1] < 1) {
...
bool Model3D::CheckValidIndex(string Type, int Index, int Check) const {
	...
	// 寻找Index是否能匹配到Line的两个Point或Face的三个Point中的一个，而不是直接判断Index是否在范围内
		...
		if (Check == m_Faces[Index - 1].Points[0].Index || Check == m_Faces[Index - 1].Points[1].Index || Check == m_Faces[Index - 1].Points[2].Index) {
			return false;
		}
	}
	return true;
...
string Model3D::ModFace(int ModFaceIndex, int OldFaceIndex, int NewFaceIndex) {
	//通过索引更改来修改面
	//检查Index是否在范围内
	if (CheckValidIndex("Face", ModFaceIndex, OldFaceIndex) || NewFaceIndex < 1 || NewFaceIndex > m_Points.size()) {
...
string Model3D::ModLine(int ModLineIndex, int OldLineIndex, int NewLineIndex) {
	//通过索引更改来修改线
	//检查Index是否在范围内
	if (CheckValidIndex("Line", ModLineIndex, OldLineIndex) || NewLineIndex < 1 || NewLineIndex > m_Points.size()) {
